...
import type { ShirtModel, Profile } from '../../types/models'
...
export default function Catalog() {
  ...
  const [authForm, setAuthForm] = useState<AuthForm>({ name: '', email: '', phone: '', password: '' })
  ...
  const getStock = (model: ShirtModel, size: string) => {
    ...
    <div className="min-h-screen bg-surface-50">
      ...
      >
        <div className="space-y-4">
          <p className="text-2xl font-bold">{formatCurrency(selectedModel?.price || 0)}</p>
          ...
          <Button className="w-full" onClick={handleAddToCart}>
            Adicionar ao Carrinho - {formatCurrency((selectedModel?.price || 0) * quantity)}
          </Button>
        </div>
      </Modal>
