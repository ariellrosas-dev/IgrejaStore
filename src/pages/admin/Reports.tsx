...
import type { Order, CashFlowEntry, OrderItem } from '../../types/models'
...
export default function Reports() {
  ...
  const chartData: ChartData[] = Object.entries(salesByDay).map(([date, value]) => ({ date, value }))
  ...
  const pieData: PieData[] = Object.entries(salesByPayment).map(([name, value]) => ({ name, value }))
  ...
  const topModelsData: TopModelData[] = Object.entries(topModels)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
