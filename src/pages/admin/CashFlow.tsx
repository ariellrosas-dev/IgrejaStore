...
import type { CashFlowEntry, CashFlowType } from '../../types/models'
import type { z } from 'zod'

const CATEGORIES: Record<CashFlowType, string[]> = {
...
export default function CashFlow() {
  ...
  const columns = [
    { header: 'Data', render: (row: CashFlowEntry) => formatDate(row.date) },
    {
      header: 'Descrição',
    ...
    <div className="p-6 space-y-4">
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Select
            label="Categoria"
            options={(CATEGORIES[selectedType as CashFlowType] || []).map(c => ({ value: c, label: c }))}
            {...register('category')}
          />
